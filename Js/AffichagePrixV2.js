        ...
        if(document.getElementById("saison").value == "haute" && document.getElementById('dureeReserv').value != "" && document.getElementById('nbTotal').value != "")
        {
            for(radio2 of document.querySelectorAll('[name=TYPE_HEBERGEMENT]'))
            {
                if(radio2.checked && radio2.value == "dur")
                {
                    ...
                    if(radio.checked)
                    {
                        calculPrixHauteSaison(typeHebergement, prixPersonne, radio.value, duree, total, null, nbEnfants, parseInt(document.getElementById('nbAdos').value, 10), nbAdultes);
                    }
                }
 ...
 * @param {int} nbAdultes nombre d'adultes
 */
function calculPrixHauteSaison(typeHebergement, prixPersonne, pension, duree, nbrePersonnes, niveau, nbEnfants, nbAdos, nbAdultes)
{
...
/**
 * Calcule et affiche le prix pour l'option sélectionnée, retourne le coût total de l'activité
 * @param {event} e élément déclencheur de l'évènement : bouton radio des frais optionnels
 * @returns {int} prixOptionTotal = coût total de l'activité
 */
function take(e)
{
    ...
    if(e.target.id == "option1")
    {
        ...
        {
            prixOptionPersonne = 2 * parseInt(document.getElementById('dureeReserv').value, 10);
    ...
    {
        prixOptionTotal = (parseInt(prixHebergement, 10) + parseFloat(prixFraisActivites, 10)) * (6 / 100);
        if(prix.innerText == "" && label.innerText == "")
        {
    ...
    {
        prixOptionTotal = 5 * parseInt(document.getElementById('nbTotal').value, 10);
        if(prix.innerText == "" && label.innerText == "")
        {
            prix.innerText = "5€";
